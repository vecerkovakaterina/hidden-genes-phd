...
orthology_group_1 = OrthologyGroup(
    ["A1", "B1"], ["homo_sapiens", "gallus_gallus"], orthology_groups
)
orthology_group_2 = OrthologyGroup(["nan", "B2"], ["gallus_gallus"], orthology_groups)
...
def test_instance_attributes():
    ...
    assert len(orthology_groups.orthology_groups_list) == 2


def test_calculate_score_without_taxon():
    orthology_group_1.calculate_score_all_groups(orthology_table, orthology_groups)
    assert orthology_group_1.score == 1.0
    assert orthology_group_2.score == 0.5


def test_calculate_score_with_taxon():
    orthology_group_1.calculate_score_all_groups(
        orthology_table, orthology_groups, target_taxon
    )
...
def test_rank_groups_by_score():
    OrthologyGroup.rank_groups_by_score(orthology_groups)
    expected_scores = [2.0, 1.5]
    actual_scores = [x.score for x in orthology_groups.orthology_groups_list]
    assert actual_scores == expected_scores


def test_delete_full_groups_without_taxon():
    orthology_group_1.calculate_score_all_groups(orthology_table, orthology_groups)
    orthology_group_1.delete_full_groups(orthology_table, orthology_groups)
    assert len(orthology_groups.orthology_groups_list) == 1
